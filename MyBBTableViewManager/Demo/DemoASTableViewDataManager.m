...
#import "DemoASTableViewDataManager.h"
#import "HomeASProductCardItem.h"

@implementation DemoASTableViewDataManager
...
//            [self.addIndexPathArray removeAllObjects];
            ...
            for (NSDictionary *entry in listData) {
                ...
                if (!error) {
                      ...
                      HomeASProductCardItem  *item = [HomeASProductCardItem itemWithModel:model
...
-(void)refreshCell:(NSInteger)actionType dataId:(NSString *)dataId {
    ...
    MyBBTableViewSection *section =[[self.tableViewDataSource.mManager sections] objectAtIndex:self.currentIndexPath.section];
    if (section && [[section items] count] > 0) {
        
    }
    //刷新
    [self.tableViewDataSource.tableNode reloadRowsAtIndexPaths:@[self.currentIndexPath]
