...
@implementation BBTableViewManager

+ (void)initialize
{
//    [BBValidation registerDefaultValidators];
//    [BBValidation registerDefaultErrorMessages];
}
...
- (id)initWithTableView:(UITableView *)tableView delegate:(id<BBTableViewManagerDelegate>)delegate
{
...
- (id)initWithTableView:(UITableView *)tableView
{
    ...
    self.style = [[BBTableViewCellStyle alloc] init];
...
{
    self[@"__NSCFConstantString"] = @"BBTableViewItemCell";
    self[@"__NSCFString"] = @"BBTableViewItemCell";
    self[@"NSString"] = @"BBTableViewItemCell";
    self[@"BBTableViewItem"] = @"BBTableViewItemCell";
...
{
    BBTableViewSection *section = self.mutableSections[indexPath.section];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)sectionIndex
{
    ...
    return ((BBTableViewSection *)self.mutableSections[sectionIndex]).items.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    BBTableViewSection *section = self.mutableSections[indexPath.section];
    BBTableViewItem *item = section.items[indexPath.row];
    ...
    BBTableViewItemCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    
    void (^loadCell)(BBTableViewItemCell *cell) = ^(BBTableViewItemCell *cell) {
        cell.tableViewManager = self;
        
        // RETableViewManagerDelegate
        //
        if ([self.delegate conformsToProtocol:@protocol(BBTableViewManagerDelegate)] && [self.delegate respondsToSelector:@selector(tableView:willLoadCell:forRowAtIndexPath:)])
        ...
        // RETableViewManagerDelegate
        //
        if ([self.delegate conformsToProtocol:@protocol(BBTableViewManagerDelegate)] && [self.delegate respondsToSelector:@selector(tableView:didLoadCell:forRowAtIndexPath:)])
    ...
    if ([cell isKindOfClass:[BBTableViewItemCell class]] && [cell respondsToSelector:@selector(loaded)] && !cell.loaded) {
        loadCell(cell);
    }
    ...
    if ([item isKindOfClass:[BBTableViewItem class]])
        cell.detailTextLabel.text = ((BBTableViewItem *)item).detailLabelText;
...
- (NSArray *)sectionIndexTitlesForTableView:(UITableView *)tableView
{
    ...
    for (BBTableViewSection *section in self.mutableSections) {
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)sectionIndex
{
    ...
    BBTableViewSection *section = self.mutableSections[sectionIndex];
...
{
    BBTableViewSection *sourceSection = self.mutableSections[sourceIndexPath.section];
    BBTableViewItem *item = sourceSection.items[sourceIndexPath.row];
    [sourceSection removeItemAtIndex:sourceIndexPath.row];
    
    BBTableViewSection *destinationSection = self.mutableSections[destinationIndexPath.section];
        ...
        if (item.deletionHandlerWithCompletion) {
            item.deletionHandlerWithCompletion(item, ^{
                ...
                for (NSInteger i = indexPath.row; i < section.items.count; i++) {
                    BBTableViewItem *afterItem = [[section items] objectAtIndex:i];
                    BBTableViewItemCell *cell = (BBTableViewItemCell *)[tableView cellForRowAtIndexPath:afterItem.indexPath];
                    cell.rowIndex--;
                }
            });
        } else {
...
{
    if ([cell isKindOfClass:[BBTableViewItemCell class]])
        [(BBTableViewItemCell *)cell cellDidDisappear];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)sectionIndex
{
    ...
    if (section.headerHeight != BBTableViewSectionHeaderHeightAutomatic) {
    ...
    if (section.headerView) {
        ...
        if (!UITableViewStyleGrouped) {
            ...
            CGSize headerRect = CGSizeMake(headerWidth, BBTableViewSectionHeaderHeightAutomatic);
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)sectionIndex
{
    ...
    if (section.footerHeight != BBTableViewSectionFooterHeightAutomatic) {
    ...
    if (section.footerView) {
        ...
        if (!UITableViewStyleGrouped) {
            ...
            CGSize footerRect = CGSizeMake(footerWidth, BBTableViewSectionFooterHeightAutomatic);
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([item respondsToSelector:@selector(setAccessoryButtonTapHandler:)]) {
        BBTableViewItem *actionItem = (BBTableViewItem *)item;
...
{
    for (BBTableViewSection *section in array)
        section.tableViewManager = self;
...
{
    for (BBTableViewSection *section in sections)
        section.tableViewManager = self;
...
{
    for (BBTableViewSection *section in otherArray)
        section.tableViewManager = self;
