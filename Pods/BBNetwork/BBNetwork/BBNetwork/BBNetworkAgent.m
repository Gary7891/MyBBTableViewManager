...
- (void)addRequest:(BBBaseRequest *)request {
    ...
    manager.operationQueue.maxConcurrentOperationCount = _config.maxCurrentRequestCount;
    ...
    [[BBNetWorkRecord sharedReord] printRequestStart:request.requestUrl header:request.requestHeaderFieldValueDictionary params:request.requestArgument mark:nil];
                                     ...
                                     parameters:param
                                       progress:NULL
                                        success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                                            [[BBNetWorkRecord sharedReord] printRequestEnd:responseObject url:request.requestUrl mark:@"GET"];
                                            [self handleRequestResult:task responseObject:responseObject error:nil];
                                        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
                                            [[BBNetWorkRecord sharedReord] printfRequestFaild:request.requestUrl mark:@"GET"];
        ...
        if (constructingBlock != nil) {
            request.sessionDataTask = [manager POST:url
                           ...
                           constructingBodyWithBlock:constructingBlock
                                       ...
                                       {
                                           [[BBNetWorkRecord sharedReord] printRequestEnd:responseObject url:request.requestUrl mark:@"POST"];
                                           [self handleRequestResult:task responseObject:responseObject error:nil];
                                       } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error)
                                       {
                                           [[BBNetWorkRecord sharedReord] printfRequestFaild:request.requestUrl mark:@"POST"];
                                           [self handleRequestResult:task responseObject:nil error:error];
                                       }];
        } else {
            request.sessionDataTask = [manager POST:url
